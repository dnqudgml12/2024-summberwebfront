...
const FreeboardDetail = () => {
  ...
  useEffect(() => {
    ...
    if (post) {
      setPost(post);
      setCommentCount(post.comments.reduce((count, comment) => count + 1 + comment.replies.length, 0));
  ...
  const handleAddReply = (commentId) => {
    const updatedPost = { ...post };
    const idnum=updatedPost.comments.find((comment) => comment.id === commentId).replies
    .length + 1;
    const newReplyObj = {
      id:idnum,
      content: newReply,
