...
const Freshmanboardedit=({id,onCancel})=>{
  ...
  */
    ...
    useEffect(() => {
      const fetchPost = async () => {
        try {
          const response = await axios.get(`http://localhost:8080/api/freshmanboard/read/${id}`); // Replace with your API endpoint
  ...
  */
    ...
    const handleUpdatePost = async () => {
      try {
        await axios.put(`http://localhost:8080/api/freshmanboard/update/${id}`, post); // Replace with your API endpoint
        const response = await axios.get(`http://localhost:8080/api/freshmanboard/read/${id}`); // Replace with your API endpoint
        setpost(response.data);
