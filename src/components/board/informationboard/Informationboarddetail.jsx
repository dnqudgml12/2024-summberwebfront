...
const Informationboarddetail = () => {
    ...
    const fetchPost = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8080/api/informationboard/read/${id}`
  ...
  const handleDeletePost = async () => {
    try {
      await axios.delete(`http://localhost:8080/api/informationboard/delete/${id}`);
  ...
  const handleAddComment = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:8080/api/informationboard/${id}/comments`,
        newCommentObj
      );
      const response = await axios.get(
        `http://localhost:8080/api/informationboard/read/${id}`
...
*/
  ...
  const handleAddReply = async (commentId) => {
    ...
    try {
      await axios.post(
        `http://localhost:8080/api/informationboard/${id}/comments/${commentId}/replies`,
        newReplyObj
      );
  ...
  */
  ...
  const handleLikePost = async () => {
    ...
    try {
      if (!liked) {
        await axios.post(`http://localhost:8080/api/informationboard/like/${id}`); //좋아요 눌린상태+1
        setLiked(true);
      } else {
        await axios.delete(`http://localhost:8080/api/informationboard/unlike/${id}`); // 좋아요 취소한 상태 -1
        setLiked(false);
      }
      const response = await axios.get(
        `http://localhost:8080/api/informationboard/read/${id}`
