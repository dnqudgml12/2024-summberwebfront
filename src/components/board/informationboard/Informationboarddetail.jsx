...
import Informationboardedit from "./Informationboardedit";

const Informationboarddetail = () => {
  const { id } = useParams();
    ...
    const fetchPost = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/api/informationboard/read/${id}`
        ); // Replace with your API endpoint
        setPost(response.data);
        setCommentCount(
          response.data.informationComment.reduce(
  ...
  const handleDeletePost = async () => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/informationboard/delete/${id}`);
  ...
  const handleAddComment = async () => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/informationboard/${id}/comments`,
        newCommentObj
      );
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/informationboard/read/${id}`
...
*/
  ...
  const handleAddReply = async (commentId) => {
    ...
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}/api/informationboard/${id}/comments/${commentId}/replies`,
        newReplyObj
      );
  ...
  */
  ...
  const handleLikePost = async () => {
    ...
    try {
      if (!liked) {
        await axios.post(`${import.meta.env.VITE_API_URL}/api/informationboard/like/${id}`); //좋아요 눌린상태+1
        setLiked(true);
      } else {
        await axios.delete(`${import.meta.env.VITE_API_URL}/api/informationboard/unlike/${id}`); // 좋아요 취소한 상태 -1
        setLiked(false);
      }
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/informationboard/read/${id}`
          ...
          {post ? (
            <>
              <SecondIndetailBody>
                ...
                {click ? (
                  <Informationboardedit id={post.id} onCancel={handleAddClick} />
                ) : (
                  <>
                ...
                <Likecommentdiv>
                  ...
                  <CommentCount>{countComments(post.informationComment)}</CommentCount>
                </Likecommentdiv>
                <Likedetaildiv>
                  {!liked ? (
                    <Likedetailbutton
                ...
                <>
                  {post.informationComment.length > 0 ? (
                    post.informationComment.map((comment) => (
...
export default Informationboarddetail;
